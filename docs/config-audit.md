...
- 审计日期：2026-09-21（1.26.1 上游核对；此前 2026-09-17 1.26.0 上游核对、2026-09-14 1.24.3 上游核对、2026-09-04 升级复核、2026-09-02 初审）
- 审计对象：`singlefile-settings-HFA.json`
- 适用版本：SingleFile 1.26.1（配置基线为 2026-09-04 由用户重新导出的 1.24.0 快照）
- 审计基准：commit `f46767b`（1.26.0 核对后状态，148 键）
- 当前状态：1.24.0 导出基线 + 1.24.3 / 1.25.0 / 1.26.0 的新增键与改名，148 键（1.26.1 未改动配置面，键集与取值均无变化）
- 方法：静态审计 + 与 1.24.0 真实导出的键级 diff + 与上游 `v1.26.1` 源码 `src/core/bg/config.js` 的 `DEFAULT_CONFIG`（148 键）键级比对，并以脚本复现扩展 `upgrade()` 的迁移逻辑做等价性验证 —— JSON 结构、内部一致性、字段语义归类；源码无法确证的语义仍标注为推断

## 结论摘要

- **结构健康**：JSON 语法有效；148 键与上游 `DEFAULT_CONFIG` 键集完全一致；无影响高保真目标的矛盾配置。发现 1 处**键之间互相矛盾**的历史遗留（`compressContent` 与归档族键，见下条），已按实际生效的格式对齐，无其他强制修改项。
- **1.26.1 上游核对（2026-09-21）**：上游本版只把内置的 single-file-core 由 1.6.5 升到 1.6.7（扩展自身只有俄语翻译与 CI 镜像两处改动）。`src/core/bg/config.js` 在 1.26.0 之后**一字未改**（该文件最后一次提交 `eb69c68` 早于 `v1.26.0` 标签），所以 `DEFAULT_CONFIG`（148 键）、`DEPRECATED_OPTION_NAMES` 改名表、`upgrade()` 迁移逻辑与 `LEGACY_FILENAME_REPLACED_CHARACTERS` 重置规则都与 1.26.0 相同。本地 148 键与 1.26.1 的 `DEFAULT_CONFIG` 键集**完全一致**（无缺失、无多余），**版本同步本身不要求改动任何键**；需要留意的是 core 1.6.6 / 1.6.7 带来的保存行为变化（见「三、发现与处置」1），以及同批核对查出的另一处与版本无关的历史遗留矛盾（见下条与「三、发现与处置」2）。
- **格式键矛盾（2026-09-21 发现并处置）**：`compressContent` 在 1.26.x 是「是否产出 ZIP / 自解压归档」的总开关（由选项页的「格式」下拉驱动），本配置为 `false`，即保存格式是**纯自包含 HTML**；于是 `selfExtractingArchive` / `extractDataFromPage` / `preventAppendedData` / `maxAppendedDataLength` / `disableCompression` 全部不可达。已把前两者由 `true` 对齐为 `false`（键集不变，148 键），使文件与选项页对「HTML」格式的写法一致、不再被静默改写；`compressContent` 保持 `false`（与 README 的「单个自包含 HTML」一致）。
...
## 一、结构总览（当前状态，148 键 = 1.24.0 导出基线 + 上游 1.24.3 / 1.25.0 / 1.26.0 演进；1.26.1 未引入配置变更，2026-09-21 另按格式对齐改了 2 个键值）
...
- 键类型分布：布尔 98（true 24 / false 74）、字符串 31（空 21 / 非空 10）、数字 13、数组 4、嵌套对象 1（`acceptHeaders`）、null 1（`customShortcut`）
...
- 2026-09-21 值调整：`selfExtractingArchive`、`extractDataFromPage` 由 `true` 改为 `false`（对齐实际生效的「HTML 格式」，见「三、发现与处置」2）；键集不变，仍为 148 键
...
- 压缩关闭：`compressHTML` / `compressCSS` / `compressContent` 均为 `false`。注意 `compressContent` 在 1.26.x 并不是「是否压缩内容」的用户选项，而是「是否产出 ZIP / 自解压归档」的总开关（选项页「格式」下拉驱动）：`false` = HTML 格式，资源内联为 `data:` URI。本配置的保存格式即由此确定为纯自包含 HTML（见「三、发现与处置」2）
...
### 归档族（当前不可达：`compressContent = false`）

- `selfExtractingArchive = false`、`extractDataFromPage = false`、`preventAppendedData = false`、`maxAppendedDataLength = 16361`、`disableCompression = false`、`createRootDirectory = false`
- 说明：这些键只在 core 的压缩 / 归档阶段被读取，而该阶段整段挂在 `compressContent` 之后（core `single-file.js` 的 `if (options.compressContent) { … processors.compression.process(…) … }`）。本配置 `compressContent = false` → 走内联（HTML）路径，上述键一律不参与行为。2026-09-21 已把其中由 UI 按格式写入的两项（`selfExtractingArchive`、`extractDataFromPage`）对齐为 `false`；其余保留上游默认值以维持键集完整。详见「三、发现与处置」2
- 该路径下仍然生效、与体积相关的去重：字节完全相同的图片按 `groupDuplicateImages = true` 共享一份数据（受 `maxSizeDuplicateImages = 1048576`（1 MB，上游默认 512 KB）上限约束），`groupDuplicateStylesheets = false` 则样式表不去重。门控与上限已由 core `processor-helper-inline.js` 源码确证；替换后的引用方式未在真实页面上逐项比对
...
1. **1.26.1 上游核对：配置面零变化，core 升级未绕过 HFA 已关闭的裁剪开关（2026-09-21，无需改动）**
   ...
   精确性说明：core 侧结论以 v1.6.7 标签下的**端状态**源码为据，未与 v1.6.5 逐行 diff，故「某项行为是否自 1.6.6 起才如此」不作断言；`v1.26.1` 标签的 `package.json` 写的是 `"single-file-core": "^1.6.5"`（caret 区间，构建时按 lockfile 解析），「实际打包 1.6.7」取自上游发布说明而非构建产物核对。

2. **格式键矛盾：`compressContent = false` 让归档族键全部失效，已按「HTML 格式」对齐（2026-09-21，已改 2 键）**
   发现：1.26.1 的选项页用**一个「格式」下拉**同时驱动三个键 —— `compressContent = fileFormatSelectInput.value.includes("zip")`、`selfExtractingArchive = …includes("self-extracting")`、`extractDataFromPage = value == "self-extracting-zip-universal"`（`src/ui/bg/ui-options.js` 的 `update()`；下拉项见 `src/ui/pages/options.html`，文案见 `_locales/en/messages.json`：HTML / self-extracting ZIP / self-extracting ZIP (universal) / ZIP）。**没有任何控件直接绑定 `compressContent` 或 `selfExtractingArchive`**，所以「`compressContent = false` + `selfExtractingArchive = true` + `extractDataFromPage = true`」这个组合**不可能由 UI 产生**，只能来自旧版本导出或手工编辑。
   成因（git 历史）：`compressContent` 在 `d47c1ee`（2026-08-20）之前一直是 `true`；2026-09-02 的 `35b299b`「优化 HFA 高保真配置：关闭压缩以保留原始资源质量」把它与 `compressHTML` / `compressCSS` 一起改为 `false` —— 当时按「是否压缩内容」理解这个键，而它在 1.26.x 实际是「是否产出 ZIP / 自解压归档」的总开关。归档路径自此静默失效，而 `selfExtractingArchive` / `extractDataFromPage` / `maxAppendedDataLength` 一直留着旧值。
   源码依据（core v1.6.7 / 扩展 v1.26.1）：压缩与归档阶段整段挂在 `compressContent` 之后 —— `single-file.js` 的 `if (options.compressContent) { … processors.compression.process(pageData, compressionOptions) … }`；资源辅助类也由它决定：`core/processor-helper.js` 的 `return options.compressContent ? getHelperClass(utilInstance) : getHelperInlineClass(utilInstance)`；MIME 判定 `core/util.js` 的 `return !options.compressContent || options.selfExtractingArchive ? "text/html" : "application/zip"`；扩展下载分支同样以它为准（`src/core/bg/downloads.js` 的 `if (message.compressContent) … else …`）。因此 `selfExtractingArchive` / `preventAppendedData` / `maxAppendedDataLength` / `disableCompression` / `createRootDirectory` 的读取点（`processors/compression/compression.js` 的 `prependHTMLData`、`buildArchive`、`getMaxAppendedDataLength`、`addFile`）在本配置下**全部不可达**。
   处置（2026-09-21，经确认「维持 HTML 格式」）：
   - `selfExtractingArchive`：`true` → `false`；`extractDataFromPage`：`true` → `false` —— 二者在 `compressContent = false` 下不参与任何行为，改后与选项页「HTML」格式的写法一致，文件因此**自洽且稳定**：此前只要在选项页改动任意一项，`update()` 就会按下拉把这两个键静默覆盖。`extractDataFromPage` 另有一个在非归档路径下可达的痕迹：当 `saveFilenameTemplateData = true` 时，core `index.js` 的 `saveFilenameTemplateData()` 会把它写进页面内嵌的 `<script type="application/json" data-single-file-options>`；本配置该键为 `false`、文件名模板不含 `{digest-sha-N}` 且 `openEditor = false`，因此这条痕迹也不存在。
   - 附带发现（不影响本配置）：core `index.js` 的 `loadOptionsFromPage()` 在 FINALIZE 阶段**无开关**地（`Object.keys(options).forEach(option => this.options[option] = options[option])`）把页面内嵌的 `data-single-file-options` JSON 覆盖回 `options`。即再保存一个「曾被 SingleFile 保存过」的页面时，页面里记录的同名字段会临时覆盖用户配置；在 `compressContent = false` 下这同样不产生行为差异。
   - `compressContent` 保持 `false`：与 README「把网页保存为单个自包含 HTML」「产出 HTML 保持原始可读格式」一致，产出仍是资源内联为 `data:` URI 的纯 HTML（`mimeType = text/html`）。
   - `preventAppendedData` / `maxAppendedDataLength` / `disableCompression` / `createRootDirectory` 保留原值：键集仍与上游 `DEFAULT_CONFIG` 完全一致（148 键不变），本文档将它们标注为不可达。
   影响提示：若日后想恢复「资源按原始字节存进 ZIP、重复资源去重、产物可复现」的自解压归档，必须把**格式切回 ZIP / 自解压 ZIP**（即 `compressContent = true`）；只把 `selfExtractingArchive` 改回 `true` 不会生效。
...
4. **1.24.3 上游核对：新增唯一键 `maxAppendedDataLength`（2026-09-14，已合入）**
   ...
   语义：自解压归档 ZIP 数据之后允许追加的数据量预算；仅在 `compressContent = true`（压缩 / 归档路径）且 `selfExtractingArchive = true`、`preventAppendedData = false` 时处于生效路径。**更正（2026-09-21）**：当时只核了后两个条件，漏掉了最外层的 `compressContent` 门 —— 本配置 `compressContent = false`，该键实际**不可达**（见第 2 条）。键值保留 `16361`（与上游默认一致，不影响键集完整性），但不再声称它在 HFA 下生效。
...
7. **大量默认关闭项**（74 个 `false` 布尔、21 个空字符串）
   判定：SingleFile 全量导出自带状态，非刻意配置，无需处理。
8. **配置漂移风险（跟踪项）**
   本文件是全量导出快照：SingleFile 升级会引入新键、改名与迁移标记；文件本身不记录适用版本，长期不更新会落后于扩展能力，直接覆盖导出又会冲掉刻意设置。1.24.0、1.24.3、1.26.0、1.26.1 各已完成一轮核对；**改名型变更是目前最大的漂移风险**（旧键被静默删除，取值不迁移），故每轮核对都必须同时检查键集与键值。1.26.1 说明并非每次升级都动配置面：该版只换了 core，配置键集零变化。后续升级时重复本流程。

## 四、跟进建议

1. 在 README 记录适用的 SingleFile 版本号（2026-09-21 已更新为：SingleFile 1.26.1）。
2. 在 README 固化「刻意设置的键」清单，与导出默认值区分（已并入「高保真策略要点」）。
3. SingleFile 升级后重新导出配置时，先与旧文件 diff，再合入新键 / 迁移项（1.24.0、1.24.3、1.26.0、1.26.1 各已执行一轮，见「三、发现与处置」）。**改名型变更需额外注意**：取值迁移与否由扩展的迁移表决定，映射为 `null` 的键会被静默删除并回落到新默认。
4. 建议将**扩展本体**升级至 1.26.1（2026-09-20 发布；本文件已按 1.26.1 核对）。1.26.1 相对 1.26.0 只换了内置 core（1.6.5 → 1.6.7）与俄语翻译，无论升级与否都不影响本配置的键集；core 侧值得知道的变化：字体面裁剪与空规则移除被修正得更彻底（两者都被本配置的 `removeUnusedFonts` / `removeUnusedStyles = false` 关掉，见「三、发现与处置」1）、归档写入顺序确定化（同一页面两次保存产出相同归档，便于按哈希判断页面是否变化）、自解压页在发现多于一个归档候选时拒绝解压。1.26.0 相对 1.24.0 的保真度变化（保存页不再能提交表单或设置 base URI，`javascript:` URI 在所有属性与命名空间中被净化；资源字节优先于声明的 content type；响应头完整转发且按大小写不敏感查找；页面文档不再受单资源大小上限约束）依旧包含在内；1.24.1–1.24.3 的修复（复合 `@font-face` 规则全部保留、iframe 内 SVG 文档内容保留、sandboxed `srcdoc` iframe 按渲染结果保存、`srcset` 保存失败时不留空属性对、BMP / GIF87a 扩展名识别修正）同理。zip.js 升至 2.15.0 后产出的归档与 1.24.0 逐字节不同 —— 但这些归档相关行为在本配置（HTML 格式）下都不触发。
...
6. **保存格式已显式定为 HTML**（2026-09-21）：`compressContent = false` + `selfExtractingArchive = false` + `extractDataFromPage = false`，与选项页「格式 = HTML」一致。若日后需要「资源按原始字节存进 ZIP、重复资源去重、产物可复现」的自解压归档，**必须改 `compressContent`（或直接用选项页的格式下拉选 ZIP / 自解压 ZIP）**，单独恢复 `selfExtractingArchive = true` 无效（见「三、发现与处置」2）。

## 五、语义待确证清单

以下键未逐一到上游源码 / 选项页确证语义，建议以 SingleFile 选项页对应控件说明为准；若与高保真目标冲突再行调整（已由上游源码确证的不在此列：`maxAppendedDataLength`、`loadDeferredContentMinZoomFactor`、`readMaffMetadata`，以及 `selfExtractingArchive` / `extractDataFromPage` / `preventAppendedData` / `disableCompression`（本配置下不可达，见「三、发现与处置」2）、`groupDuplicateImages` + `maxSizeDuplicateImages`（去重门控与体积上限））：
